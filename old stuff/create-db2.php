...
//		Alle Zeilen verarbeiten
		...
		foreach($zeilen as $zeile)
		{
			...
			{
				$pos++;
				if($pos<2)	// Datumsbereich
				{
					if ($pos==1)
					{
						$Datestring = explode (" ",$wert); // ; separated Zerlegen
						$Times = explode (":",$Datestring[1]); // ; separated Zerlegen
						$sDate=$Datestring[1]." ".$Datestring[0];
						if (SHORT_ECHO) {echo '/'.$sDate."/";}
						$rDate = strtotime(trim($sDate));
						if (SHORT_ECHO) {echo $rDate."/".strftime('%Y-%m-%d-%H:%M:%S', $rDate);}
					}
