...
$db_file = $loggerdir."/dbase/gen_".$logger."_v2.sqlite";
...
// ELSE Create DB, Open DB, Build Table, 
	...
	$result = $db->exec('CREATE TABLE measurement (ID INTEGER PRIMARY KEY, SensorID VARCHAR, Value REAL, AnswerDate INTEGER, RequestDate INTEGER)');
...
// 			DB write , für jeden Wert eine Zeile generieren per execute
			...
			if ($ID>1)
			{
				...
				{
					$sValue="'$Value[$index]'";
					$query = "INSERT INTO measurement (SensorID, Value, AnswerDate, RequestDate) VALUES ($SensorID[$index], $sValue, $rDate, $rDate)";
					$result = $db->exec($query);
				}
			} else {
				for ($index=1;$index<$Index;$index++)
				{
					$SensorID[$index]="'$Value[$index]'";
				}
			}	
			if (SHORT_ECHO) {echo "\n";}
		}
